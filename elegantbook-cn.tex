...
我们将式(\ref{eq:旋转变换})称为旋转角为 $180^{\circ}$ 的\textcolor{third}{\bf 旋转变换}的表达式, 它建立了平面内的每个点 $P$ 到其对应点 $P^{\prime}$ 的对应关系, 我们称 $P^{\prime}$ 是 $P$ 在这个旋转变换作用下的像.
...
我们可以对式\ref{eq:旋转变换}进行简单改写，如式\ref{eq:旋转变换的方程}所示：
...
显然，在二维平面内旋转$180^{\circ}$这一变换可以由式\ref{eq:旋转变换示例矩阵}唯一确定。反过来，该矩阵也可以唯一确定这个变换。因此我们把这个矩阵称为该变换对应的\textcolor{third}{\bf 变换矩阵}。
...
我们将式(\ref{eq:反射变换})称为关于 $x$ 轴的\textcolor{third}{\bf 反射变换}的表达式。我们可以对式\ref{eq:反射变换}进行简单改写，如式\ref{eq:反射变换的方程}所示：
...
\subsection{函数的定义}
\begin{definition}[函数]
函数是一种数学工具，它将一个输入值映射到一个输出值。函数可以用公式表示，例如 $f(x) = x^2$。这个公式表示，当输入值为 $x$ 时，输出值为 $x$ 的平方。例如当输入值为 $2$ 时，输出值为 $2$ 的平方也就是$4$。
\end{definition}

\subsection{函数的定义域和值域}

\begin{itemize}
    \item 函数的定义域是指函数可以接受的输入值的集合。在上面的例子中，函数 $f(x) = x^2$ 的定义域是所有实数。有些函数的定义域可能会受到限制，例如 $\sqrt{x}$ 只能接受非负实数作为输入。

    \item 函数的值域是指函数可能输出的值的集合。在上面的例子中，函数 $f(x) = x^2$ 的值域是所有非负实数。有些函数的值域可能会受到限制，例如 $\sin x$ 的值域是 $[-1, 1]$。
