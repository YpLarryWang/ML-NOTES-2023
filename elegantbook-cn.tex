...
\subsection{高斯消元法}
\label{subsec:origin_of_matrix-from_equation-set-to-matrix}
...
矩阵提供了一种组织方程组系数的方式，通过矩阵运算可以方便地求解方程组。这启发了矩阵理论的产生和发展。

\subsection{使用Python解方程组}
\label{subsec:origin_of_matrix-solve-equation-set-with-python}

使用Python可以很轻松地解出第\ref{subsec:origin_of_matrix-from_equation-set-to-matrix}节中的方程组。
...
我们将式(\ref{eq:旋转变换})称为旋转角为 $180^{\circ}$ 的旋转变换的表达式, 它建 立了平面内的每个点 $P$ 到其对应点 $P^{\prime}$ 的对应关系, 我们称 $P^{\prime}$ 是 $P$ 在这个旋转变换作用下的像.
