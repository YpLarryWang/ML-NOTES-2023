...
\begin{exercise}
    ...
    \item   设向量 $\boldsymbol{\alpha}=\left(\begin{array}{l}1 \\ 2\end{array}\right), \boldsymbol{\beta}=\left(\begin{array}{l}2 \\ 1\end{array}\right)$. 
            ...
            \begin{tikzpicture}[scale=1, transform shape]
              ...
              \begin{scope}[shift={(0,0)}]
                ...
                \draw[thick,->,blue] (0,0) -- (1,2) node[anchor=south] {$\boldsymbol{\alpha}$};
                \draw[thick,->,red] (0,0) -- (2,1) node[anchor=west] {$\boldsymbol{\beta}$};

                \draw[thick,-,blue,dashed] (1,2) -- (3,3);
                \draw[thick,-,red,dashed] (2,1) -- (3,3);
              
                % Draw alpha + beta
                \draw[thick,->,mygreen] (0,0) -- (3,3) node[anchor=south] {$\boldsymbol{\alpha}+\boldsymbol{\beta}$};
              
                % Draw reflection of alpha + beta
                \draw[thick,->,mygreen,dashed] (0,0) -- (3,-3) node[anchor=north] {$\text{Reflect}_{x}(\boldsymbol{\alpha}+\boldsymbol{\beta})$};
              ...
              \begin{scope}[shift={(7,0)}]
                ...
                % Draw original vectors alpha and beta
                \draw[thick,->,blue] (0,0) -- (1,2) node[anchor=north] {$\boldsymbol{\alpha}$};
                \draw[thick,->,red] (0,0) -- (2,1) node[anchor=west] {$\boldsymbol{\beta}$};
              
                % Draw reflection of alpha and beta
                \draw[thick,->,blue,dashed] (0,0) -- (1,-2) node[anchor=north] {$\text{Reflect}_{x}(\boldsymbol{\alpha})$};
                \draw[thick,->,red,dashed] (0,0) -- (2,-1) node[anchor=west] {$\text{Reflect}_{x}(\boldsymbol{\beta})$};
                ...
                \draw[thick,->,mygreen] (0,0) -- (3,-3) node[anchor=north] {$\text{Reflect}_{x}(\boldsymbol{\alpha}) + \text{Reflect}_{x}(\boldsymbol{\beta})$};
  ...
  \item 设向量 $\boldsymbol{u}=\left(\begin{array}{l}1 \\ 2\end{array}\right)$ ，
        ...
        \begin{enumerate}
          \item 这两个过程的结果相同吗?
          \item 如果把求$\boldsymbol{u}$的反射的变换记作$\text{Stretch}_{x}(\boldsymbol{u})$，
          把求$2\boldsymbol{u}$的反射的变换记作$\text{Stretch}_{x}(2\boldsymbol{u})$，
          那么，这些变换的坐标变换矩阵分别是什么？这些变换的矩阵有什么关系？
        \end{enumerate}
...
设 $A$ 是一个二阶矩阵, $\boldsymbol{\alpha}, \boldsymbol{\beta}$ 是平面上的任意两个向量, $\lambda$ 是一个任意实 数, 则
\begin{enumerate}
  \item $\boldsymbol{A}(\lambda \boldsymbol{\alpha})=\lambda \boldsymbol{A} \boldsymbol{\alpha}$;
  \item $\boldsymbol{A}(\boldsymbol{\alpha}+\boldsymbol{\beta})=\boldsymbol{A} \boldsymbol{\alpha}+\boldsymbol{A} \boldsymbol{\beta}$.
\end{enumerate}
...
幂函数 $y=x^\alpha$ 和指数函数 $y=a^x$ 都涉及乘幂, 而在中学数 课程中只给出了有理指数乘幂的定义. 这里我们指出，无理数也可以作为幂指数。\textcolor{second}{无理指数幂和有理指数幂一起构成了实数指数幂，并保持有理指数幂的基本性质。}
